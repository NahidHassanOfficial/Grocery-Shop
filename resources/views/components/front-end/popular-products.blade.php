                         ...
                         <div class="card-body">
                             ...
                             <div class="text-center position-relative ">
                                 <div class=" position-absolute top-0 start-0">
                                     <span class="badge bg-danger">{{ $product->tag ? $product->tag : '' }}</span>
                                 </div>
                                 <a href="{{ route('product.view', [$product->categorySlug, $product->slug]) }}"> <img
                                         src="{{ asset('images/products/' . json_decode($product->image)[0]) }}"
                                         alt="Grocery Ecommerce Template" class="mb-3 img-fluid"></a>

                                 <div class="card-product-action">
                                     <a href="#!" class="btn-action" data-bs-toggle="modal"
                                         data-bs-target="#productViewModal" onclick="productId({{ $product->id }})"><i
                             ...
                             <div class="text-small mb-1"><a href="{{ route('category.view', $product->categorySlug) }}"
                                     class="text-decoration-none text-muted"><small>{{ $product->categoryName }}</small></a>
                             </div>
                             <h2 class="fs-6"><a
                                     href="{{ route('product.view', [$product->categorySlug, $product->slug]) }}"
